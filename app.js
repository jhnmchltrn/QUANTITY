        ...
        document.getElementById('inputForm').addEventListener('submit', function (e) {
            ...
            // If all input fields are not empty, go here
                ...
                // If it doesn't exist yet, go here
                    ...
                    inventoryItems.push({ name: item, quantity: quantity });
                    ...
                    // adding event listener to quantity for editing
                    tdElementForQty.addEventListener('click', function () {
                        const newText = prompt("Enter new quantity:", quantity);
                        if (newText !== null) {
                            tdElementForQty.textContent = newText;
                            // update quantity in inventoryItems array
                            const index = inventoryItems.findIndex(item => item.name === item);
                            if (index !== -1) {
                                inventoryItems[index].quantity = newText;
                            }
                        }
                    });
